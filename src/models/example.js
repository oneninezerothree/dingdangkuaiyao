...
export default {
    ...
    *getdetail({ detail}, { call, put }) { 
     ...
     *toggletab({ toggletab}, { call, put }) { 
      ...
      //这里的put表示存储在当前命名空间example中，通过save方法存在当前state中
       ...
       yield put({
          type: 'tog',   //这个就是调用reducers中的方法进行跟新当前命名空间state的数据
          toggletab:toggletab
       });
     },
  },
  ...
  reducers: {
    ...
    ret(state, action) { //这里的state是当前总的state，这里的action包含了上面传递的参数和type
      return { ...state, detail:action.detail }; //这里用ES6语法来更新当前state中num的值
    },
    tog(state, action) { //这里的state是当前总的state，这里的action包含了上面传递的参数和type
      return { ...state, toggletab:action.toggletab }; //这里用ES6语法来更新当前state中num的值
    },
  },
